...
export function Sidebar({ sections, isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      {/* Mobile overlay */}
      <div
        className={`fixed inset-0 bg-ink/40 backdrop-blur-sm z-[60] lg:hidden transition-opacity duration-300 ${
                      ...
                      return (
                        <li key={item.href}>
                          <Link
                            ...
                            className={`flex items-center gap-3 py-2.5 px-3 rounded-lg font-medium text-sm transition-colors ${
                              active
                        ...
                        {item.badge ? (
                          <span
                            className={`min-w-[20px] text-center text-[11px] px-1.5 py-0.5 rounded-full font-semibold ${
